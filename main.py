...
def dedust_trade_to_buy(tr: Dict[str, Any], token_addr: str, pool_addr: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if not tx:
        tx = str(tr.get("transactionHash") or tr.get("transaction_hash") or tr.get("tx_hash") or tr.get("txid") or "").strip()
    # If still missing but we have lt and pool address, resolve via TonAPI account transactions
    if not tx and lt and pool_addr:
        resolved = resolve_tx_hash_by_lt(pool_addr, lt)
        if resolved:
            tx = resolved
...
TELEGRAM_RE = re.compile(r"(https?://t\.me/[A-Za-z0-9_]{3,32}|t\.me/[A-Za-z0-9_]{3,32}|@[A-Za-z0-9_]{3,32})", re.IGNORECASE)
...
def tx_hash_to_hex(h: str) -> Optional[str]:
    """Tonviewer expects 64-hex tx hash. Accepts hex or base64/base64url."""
    if not h:
        return None
    h = h.strip()
    if re.fullmatch(r"[0-9a-fA-F]{64}", h):
        return h.lower()
    # base64url / base64 (TonAPI uses base64url for hashes)
    try:
        pad = "=" * ((4 - (len(h) % 4)) % 4)
        b = base64.urlsafe_b64decode(h + pad)
        if len(b) == 32:
            return b.hex()
    except Exception:
        pass
...
def tonviewer_tx_url(tx_hash_any: str) -> Optional[str]:
    hx = tx_hash_to_hex(tx_hash_any)
    return f"https://tonviewer.com/transaction/{hx}" if hx else None

def resolve_tx_hash_by_lt(account_addr: str, lt: str) -> Optional[str]:
    """Resolve tx hash for an account by logical time using TonAPI."""
    if not account_addr or not lt:
        return None
    try:
        txs = tonapi_account_transactions(account_addr, limit=40)
        for t in txs:
            if str(t.get("lt")) == str(lt):
                h = t.get("hash") or t.get("transaction_id", {}).get("hash")
                if h:
                    return str(h)
    except Exception:
        return None
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tg_link = normalize_telegram_link(text)
    await configure_group_token(target_chat_id, addr, context, reply_to_chat=chat.id, telegram_link=tg_link)

async def configure_group_token(chat_id: int, jetton: str, context: ContextTypes.DEFAULT_TYPE, reply_to_chat: int, telegram_link: Optional[str] = None):
    g = get_group(chat_id)
    # 1 token per group: confirm replace if exists and different
    existing = g.get("token") or None
    # If same token already set, allow updating Telegram link without resetting tracking
    if existing and existing.get("address") == jetton and telegram_link:
        existing["telegram"] = telegram_link
        save_groups()
        try:
            await context.bot.send_message(chat_id=reply_to_chat, text="✅ Telegram link saved for this token.", disable_web_page_preview=True)
        except Exception:
            pass
        return
    if existing and existing.get("address") != jetton:
        ...
        return
    await _set_token_now(chat_id, jetton, context, reply_to_chat, telegram_link)
...
async def on_replace_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("REPL_"):
        ...
        if not await is_admin(context.bot, chat.id, user.id):
            await q.answer("Admins only.", show_alert=True)
            return
        await _set_token_now(target_chat_id, jetton, context, chat.id, None)
        try:
...
async def _set_token_now(chat_id: int, jetton: str, context: ContextTypes.DEFAULT_TYPE, reply_chat_id: int, telegram_link: Optional[str] = None):
    ...
    for chat_id, g in items:
        ...
        if settings.get("enable_dedust", True) and token.get("dedust_pool"):
            ...
            try:
                ...
                for tr in trades:
                    b = dedust_trade_to_buy(tr, token["address"], pool_addr=pool)
                    if not b:
                        continue
...
async def post_buy(app: Application, chat_id: int, token: Dict[str, Any], b: Dict[str, Any], source: str):
    ...
    tx_url = tonviewer_tx_url(tx) if tx else None
